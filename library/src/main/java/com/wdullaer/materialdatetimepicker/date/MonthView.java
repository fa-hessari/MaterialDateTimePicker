...
public abstract class MonthView extends View {
  ...
  public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
    ...
    mDayOfWeekTypeface = res.getString(R.string.fh_gig_mdtp_day_of_week_label_typeface);
    mMonthTitleTypeface = res.getString(R.string.fh_gig_mdtp_sans_serif);

    boolean darkTheme = mController != null && mController.isThemeDark();
    if (darkTheme) {
      mDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_text_normal_dark_theme);
      mMonthDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_month_day_dark_theme);
      mDisabledDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_text_disabled_dark_theme);
      mHighlightedDayTextColor = ContextCompat.getColor(context,
          R.color.fh_gig_mdtp_date_picker_text_highlighted_dark_theme);
    } else {
      mDayTextColor = ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_text_normal);
      mMonthDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_month_day);
      mDisabledDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_text_disabled);
      mHighlightedDayTextColor =
          ContextCompat.getColor(context, R.color.fh_gig_mdtp_date_picker_text_highlighted);
    }
    mSelectedDayTextColor = ContextCompat.getColor(context, R.color.fh_gig_mdtp_white);
    mTodayNumberColor = mController.getAccentColor();
    mMonthTitleColor = ContextCompat.getColor(context, R.color.fh_gig_mdtp_white);

    mStringBuilder = new StringBuilder(50);

    MINI_DAY_NUMBER_TEXT_SIZE = res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_day_number_size);
    MONTH_LABEL_TEXT_SIZE = res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_month_label_size);
    MONTH_DAY_LABEL_TEXT_SIZE =
        res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_month_day_label_text_size);
    MONTH_HEADER_SIZE =
        res.getDimensionPixelOffset(R.dimen.fh_gig_mdtp_month_list_item_header_height);
    MONTH_HEADER_SIZE_V2 =
        res.getDimensionPixelOffset(R.dimen.fh_gig_mdtp_month_list_item_header_height_v2);
    DAY_SELECTED_CIRCLE_SIZE =
        mController.getVersion() == DatePickerDialog.Version.VERSION_1 ? res.getDimensionPixelSize(
            R.dimen.fh_gig_mdtp_day_number_select_circle_radius)
            : res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_day_number_select_circle_radius_v2);
    DAY_HIGHLIGHT_CIRCLE_SIZE =
        res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_day_highlight_circle_radius);
    DAY_HIGHLIGHT_CIRCLE_MARGIN =
        res.getDimensionPixelSize(R.dimen.fh_gig_mdtp_day_highlight_circle_margin);

    if (mController.getVersion() == DatePickerDialog.Version.VERSION_1) {
      mRowHeight =
          (res.getDimensionPixelOffset(R.dimen.fh_gig_mdtp_date_picker_view_animator_height)
              - getMonthHeaderSize()) / MAX_NUM_ROWS;
    } else {
      mRowHeight =
          (res.getDimensionPixelOffset(R.dimen.fh_gig_mdtp_date_picker_view_animator_height_v2)
              - getMonthHeaderSize()
    ...
    mEdgePadding = mController.getVersion() == DatePickerDialog.Version.VERSION_1 ? 0
        : context.getResources()
            .getDimensionPixelSize(R.dimen.fh_gig_mdtp_date_picker_view_animator_padding_v2);
  ...
  @NonNull private String getMonthAndYearString() {
    ...
    if (Build.VERSION.SDK_INT < 18) {
      pattern = getContext().getResources().getString(R.string.fh_gig_mdtp_date_v1_monthyear);
    } else {
