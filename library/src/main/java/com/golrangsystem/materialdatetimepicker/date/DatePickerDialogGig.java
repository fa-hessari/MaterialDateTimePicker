...
import com.golrangsystem.materialdatetimepicker.R;
 ...
 */
public class DatePickerDialogGig extends DialogFragment
    implements OnClickListener, DatePickerControllerGig {
  ...
  private OnDateSetListenerGig mCallBack;
  private HashSet<OnDateChangedListener> mListeners = new HashSet<>();
  private DialogInterface.OnCancelListener mOnCancelListener;
  private DialogInterface.OnDismissListener mOnDismissListener;
  private AccessibleDateAnimatorGig mAnimator;
  private TextView mDatePickerHeaderView;
  private LinearLayout mMonthAndDayView;
  private TextView mSelectedMonthTextView;
  private TextView mSelectedDayTextView;
  private TextView mYearView;
  private DayPickerGroupGig mDayPickerView;
  private YearPickerViewGig mYearPickerViewGig;
  ...
  private Calendar mCalendar = UtilsGig.trimToMidnight(Calendar.getInstance(getTimeZone()));
  private int mWeekStart = mCalendar.getFirstDayOfWeek();
  private Locale mLocale = Locale.getDefault();
  private DefaultDateRangeLimiterGig mDefaultLimiter = new DefaultDateRangeLimiterGig();
  private DateRangeLimiterGig mDateRangeLimiterGig = mDefaultLimiter;
  ...
  public DatePickerDialogGig() {
    // Empty constructor required for dialog fragment.
  }

  /**
   * Create a new DatePickerDialogGig instance with a specific initial selection.
   *
   * @param callBack How the parent is notified that the date is set.
   * @param year The initial year of the dialog.
   * @param monthOfYear The initial month of the dialog.
   * @param dayOfMonth The initial day of the dialog.
   * @return a new DatePickerDialogGig instance.
   */
  public static DatePickerDialogGig newInstance(OnDateSetListenerGig callBack, int year,
      int monthOfYear,
      int dayOfMonth) {
    DatePickerDialogGig ret = new DatePickerDialogGig();
    ret.initialize(callBack, year, monthOfYear, dayOfMonth);
    return ret;
  }

  /**
   * Create a new DatePickerDialogGig instance initialised to the current system date.
   *
   * @param callback How the parent is notified that the date is set.
   * @return a new DatePickerDialogGig instance
   */
  @SuppressWarnings("unused") public static DatePickerDialogGig newInstance(
      OnDateSetListenerGig callback) {
    Calendar now = Calendar.getInstance();
    return DatePickerDialogGig.newInstance(callback, now);
  }
  ...
  public void initialize(OnDateSetListenerGig callBack, Calendar initialSelection) {
    mCallBack = callBack;
    mCalendar = UtilsGig.trimToMidnight((Calendar) initialSelection.clone());
  ...
  public void initialize(OnDateSetListenerGig callBack, int year, int monthOfYear, int dayOfMonth) {
  ...
  @Override public void onCreate(Bundle savedInstanceState) {
    ...
    if (Build.VERSION.SDK_INT < 18) {
      VERSION_2_FORMAT = new SimpleDateFormat(activity.getResources()
          .getString(R.string.fh_gig_mdtp_date_v2_daymonthyear),
          mLocale);
    } else {
      VERSION_2_FORMAT =
          new SimpleDateFormat(DateFormat.getBestDateTimePattern(mLocale, "EEEMMMdd"), mLocale);
    }
  ...
  @Override public void onSaveInstanceState(@NonNull Bundle outState) {
    ...
    if (mCurrentView == MONTH_AND_DAY_VIEW) {
      ...
      listPosition = mYearPickerViewGig.getFirstVisiblePosition();
      outState.putInt(KEY_LIST_POSITION_OFFSET, mYearPickerViewGig.getFirstPositionOffset());
    }
    ...
    outState.putParcelable(KEY_DATERANGELIMITER, mDateRangeLimiterGig);
  ...
  @Override public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
    ...
    if (mScrollOrientation == null) {
      mScrollOrientation =
          mVersion == Version.VERSION_1 ? ScrollOrientation.VERTICAL : ScrollOrientation.HORIZONTAL;
    }
    if (savedInstanceState != null) {
      ...
      highlightedDays =
          (HashSet<Calendar>) savedInstanceState.getSerializable(KEY_HIGHLIGHTED_DAYS);
      ...
      mScrollOrientation =
          (ScrollOrientation) savedInstanceState.getSerializable(KEY_SCROLL_ORIENTATION);
      mTimezone = (TimeZone) savedInstanceState.getSerializable(KEY_TIMEZONE);
      mDateRangeLimiterGig = savedInstanceState.getParcelable(KEY_DATERANGELIMITER);
      ...
      setLocale((Locale) savedInstanceState.getSerializable(KEY_LOCALE));
            ...
            and mDateRangeLimiterGig are the same reference, so that the config methods actually
            affect the behaviour of the picker (in the unlikely event the user reconfigures
            the picker when it is shown)
             */
      if (mDateRangeLimiterGig instanceof DefaultDateRangeLimiterGig) {
        mDefaultLimiter = (DefaultDateRangeLimiterGig) mDateRangeLimiterGig;
      } else {
        mDefaultLimiter = new DefaultDateRangeLimiterGig();
      }
    }

    mDefaultLimiter.setController(this);

    int viewRes = mVersion == Version.VERSION_1 ? R.layout.fh_gig_mdtp_date_picker_dialog
        : R.layout.fh_gig_mdtp_date_picker_dialog_v2;
    View view = inflater.inflate(viewRes, container, false);
    // All options have been set at this point: round the initial selection if necessary
    mCalendar = mDateRangeLimiterGig.setToNearestDate(mCalendar);

    mDatePickerHeaderView = view.findViewById(R.id.fh_gig_mdtp_date_picker_header);
    mMonthAndDayView = view.findViewById(R.id.fh_gig_mdtp_date_picker_month_and_day);
    mMonthAndDayView.setOnClickListener(this);
    mSelectedMonthTextView = view.findViewById(R.id.fh_gig_mdtp_date_picker_month);
    mSelectedDayTextView = view.findViewById(R.id.fh_gig_mdtp_date_picker_day);
    mYearView = view.findViewById(R.id.fh_gig_mdtp_date_picker_year);
    mYearView.setOnClickListener(this);

    final Activity activity = requireActivity();
    mDayPickerView = new DayPickerGroupGig(activity, this);
    mYearPickerViewGig = new YearPickerViewGig(activity, this);

    // if theme mode has not been set by java code, check if it is specified in Style.xml
    if (!mThemeDarkChanged) {
      mThemeDark = UtilsGig.isDarkTheme(activity, mThemeDark);
    }

    Resources res = getResources();
    mDayPickerDescription = res.getString(R.string.fh_gig_mdtp_day_picker_description);
    mSelectDay = res.getString(R.string.fh_gig_mdtp_select_day);
    mYearPickerDescription = res.getString(R.string.fh_gig_mdtp_year_picker_description);
    mSelectYear = res.getString(R.string.fh_gig_mdtp_select_year);

    int bgColorResource = mThemeDark ? R.color.fh_gig_mdtp_date_picker_view_animator_dark_theme
        : R.color.fh_gig_mdtp_date_picker_view_animator;
    int bgColor = ContextCompat.getColor(activity, bgColorResource);
    view.setBackgroundColor(bgColor);

    mAnimator = view.findViewById(R.id.fh_gig_mdtp_animator);
    mAnimator.addView(mDayPickerView);
    mAnimator.addView(mYearPickerViewGig);
    ...
    Button okButton = view.findViewById(R.id.fh_gig_mdtp_ok);
    ...
    if (mDatePickerHeaderView != null) {
      mDatePickerHeaderView.setBackgroundColor(UtilsGig.darkenColor(mAccentColor));
    }
    view.findViewById(R.id.fh_gig_mdtp_day_picker_selected_date_layout)
    ...
    if (getDialog() == null) {
      view.findViewById(R.id.fh_gig_mdtp_done_background)
    ...
    if (listPosition != -1) {
      if (currentView == MONTH_AND_DAY_VIEW) {
        ...
        mYearPickerViewGig.postSetSelectionFromTop(listPosition, listPositionOffset);
      }
    }
    ...
    if (hasShamsiButton) {
      ...
      if (onChangeToShamsiClickListener != null) {
        changeToShamsiBt.setOnClickListener(v -> {
          onChangeToShamsiClickListener.onChangeToShamsiClicked(DatePickerDialogGig.this,
    ...
    mHapticFeedbackControllerGig = new HapticFeedbackControllerGig(activity);
    return view;
  }
    ...
    switch (viewIndex) {
      case MONTH_AND_DAY_VIEW:
        if (mVersion == Version.VERSION_1) {
          ObjectAnimator pulseAnimator = UtilsGig.getPulseAnimator(mMonthAndDayView, 0.9f, 1.05f);
          if (mDelayAnimation) {
        ...
        UtilsGig.tryAccessibilityAnnounce(mAnimator, mSelectDay);
        break;
      case YEAR_VIEW:
        if (mVersion == Version.VERSION_1) {
          ObjectAnimator pulseAnimator = UtilsGig.getPulseAnimator(mYearView, 0.85f, 1.1f);
          if (mDelayAnimation) {
            pulseAnimator.setStartDelay(ANIMATION_DELAY);
            mDelayAnimation = false;
          }
          mYearPickerViewGig.onDateChanged();
          if (mCurrentView != viewIndex) {
        ...
        UtilsGig.tryAccessibilityAnnounce(mAnimator, mSelectYear);
        break;
    }
  }
  ...
  private void updateDisplay(boolean announce) {
      ...
      if (mDatePickerHeaderView != null) {
        if (mTitle != null) {
          ...
          mDatePickerHeaderView.setText(
              mCalendar.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG, mLocale));
        }
      }
    ...
    if (announce) {
      ...
      UtilsGig.tryAccessibilityAnnounce(mAnimator, fullDateText);
    }
  }
   ...
   */
  @SuppressWarnings("unused") public void autoDismiss(boolean autoDismiss) {
   ...
   */
  @SuppressWarnings("unused") public Calendar getMinDate() {
   ...
   */
  @SuppressWarnings("unused") public Calendar getMaxDate() {
   ...
   */
  @SuppressWarnings("unused") public Calendar[] getHighlightedDays() {
   ...
   */
  @SuppressWarnings("unused") public void setHighlightedDays(Calendar[] highlightedDays) {
    for (Calendar highlightedDay : highlightedDays) {
      this.highlightedDays.add(UtilsGig.trimToMidnight((Calendar) highlightedDay.clone()));
    }
    if (mDayPickerView != null) mDayPickerView.onChange();
  }

  @Override public boolean isHighlighted(int year, int month, int day) {
    Calendar date = Calendar.getInstance(getTimeZone());
    date.set(Calendar.YEAR, year);
    date.set(Calendar.MONTH, month);
    date.set(Calendar.DAY_OF_MONTH, day);
    UtilsGig.trimToMidnight(date);
   ...
   * restriction is set
   */
  @SuppressWarnings("unused") public Calendar[] getSelectableDays() {
  ...
  /**
   * @return an Array of Calendar objects containing the list of days that are not selectable. null
   * if no restriction is set
   */
  @SuppressWarnings("unused") public Calendar[] getDisabledDays() {
   ...
   */
  @SuppressWarnings("unused") public void setDisabledDays(Calendar[] disabledDays) {
    mDefaultLimiter.setDisabledDays(disabledDays);
    if (mDayPickerView != null) mDayPickerView.onChange();
  }

  /**
   * Provide a DateRangeLimiterGig for full control over which dates are enabled and disabled in the
   * picker
   *
   * @param dateRangeLimiterGig An implementation of the DateRangeLimiterGig interface
   */
  @SuppressWarnings("unused") public void setDateRangeLimiter(
      DateRangeLimiterGig dateRangeLimiterGig) {
    mDateRangeLimiterGig = dateRangeLimiterGig;
  }
  ...
  /**
   * Get which way the user needs to swipe to switch months in the MonthViewGig
   *
  ...
  /**
   * Set which way the user needs to swipe to switch months in the MonthViewGig
   *
   ...
   *
   * @return OnDateSetListenerGig the callback
   */
  @SuppressWarnings("unused") public OnDateSetListenerGig getOnDateSetListener() {
    return mCallBack;
  }

  @SuppressWarnings("unused") public void setOnDateSetListener(OnDateSetListenerGig listener) {
    mCallBack = listener;
  }
  ...
  private Calendar adjustDayInMonthIfNeeded(Calendar calendar) {
    ...
    return mDateRangeLimiterGig.setToNearestDate(calendar);
  }

  @Override public void onClick(View v) {
    tryVibrate();
    if (v.getId() == R.id.fh_gig_mdtp_date_picker_year) {
      setCurrentView(YEAR_VIEW);
    } else if (v.getId() == R.id.fh_gig_mdtp_date_picker_month_and_day) {
  ...
  @Override public MonthAdapterGig.CalendarDay getSelectedDay() {
    return new MonthAdapterGig.CalendarDay(mCalendar, getTimeZone());
  }

  @Override public Calendar getStartDate() {
    return mDateRangeLimiterGig.getStartDate();
  }

  @Override public Calendar getEndDate() {
    return mDateRangeLimiterGig.getEndDate();
  }

  @Override public int getMinYear() {
    return mDateRangeLimiterGig.getMinYear();
  }

  @Override public int getMaxYear() {
    return mDateRangeLimiterGig.getMaxYear();
  }

  @Override public boolean isOutOfRange(int year, int month, int day) {
    return mDateRangeLimiterGig.isOutOfRange(year, month, day);
  }
   ...
   */
  @SuppressWarnings("DeprecatedIsStillUsed") @Deprecated public void setTimeZone(
      TimeZone timeZone) {
    mTimezone = timeZone;
  ...
  public void notifyOnDateListener() {
    if (mCallBack != null) {
      mCallBack.onDateSetGig(DatePickerDialogGig.this, mCalendar.get(Calendar.YEAR),
   ...
   */
  public interface OnDateSetListenerGig {
    /**
     ...
     */
    void onDateSetGig(DatePickerDialogGig view, int year, int monthOfYear, int dayOfMonth);
  }
  ...
  public interface OnChangeToShamsiClickListener {
    void onChangeToShamsiClicked(DatePickerDialogGig view, int year, int monthOfYear,
        int dayOfMonth);
  }
}
