...
class DefaultDateRangeLimiterGig implements DateRangeLimiterGig {
  @SuppressWarnings("WeakerAccess")
  public static final Parcelable.Creator<DefaultDateRangeLimiterGig> CREATOR =
      new Parcelable.Creator<DefaultDateRangeLimiterGig>() {
        public DefaultDateRangeLimiterGig createFromParcel(Parcel in) {
          return new DefaultDateRangeLimiterGig(in);
    }

        public DefaultDateRangeLimiterGig[] newArray(int size) {
          return new DefaultDateRangeLimiterGig[size];
    }
  };
  private static final int DEFAULT_START_YEAR = 1900;
  private static final int DEFAULT_END_YEAR = 2100;
  private transient DatePickerControllerGig mController;
  ...
  DefaultDateRangeLimiterGig() {
  }

  @SuppressWarnings({ "unchecked", "WeakerAccess" }) public DefaultDateRangeLimiterGig(Parcel in) {
    mMinYear = in.readInt();
    mMaxYear = in.readInt();
  ...
  void setController(@NonNull DatePickerControllerGig controller) {
  ...
  void setMinDate(@NonNull Calendar calendar) {
    mMinDate = UtilsGig.trimToMidnight((Calendar) calendar.clone());
  }
  ...
  void setMaxDate(@NonNull Calendar calendar) {
    mMaxDate = UtilsGig.trimToMidnight((Calendar) calendar.clone());
  }
  ...
  void setSelectableDays(@NonNull Calendar[] days) {
    for (Calendar selectableDay : days) {
      this.selectableDays.add(UtilsGig.trimToMidnight((Calendar) selectableDay.clone()));
    }
  }
  ...
  void setDisabledDays(@NonNull Calendar[] days) {
    for (Calendar disabledDay : days) {
      this.disabledDays.add(UtilsGig.trimToMidnight((Calendar) disabledDay.clone()));
    }
  }
  ...
  @Override public int getMinYear() {
    ...
    return mMinDate != null && mMinDate.get(Calendar.YEAR) > mMinYear ? mMinDate.get(Calendar.YEAR)
        : mMinYear;
  }
  ...
  @Override public int getMaxYear() {
    ...
    return mMaxDate != null && mMaxDate.get(Calendar.YEAR) < mMaxYear ? mMaxDate.get(Calendar.YEAR)
        : mMaxYear;
  }

  @Override public @NonNull Calendar getStartDate() {
  ...
  @Override public @NonNull Calendar getEndDate() {
  ...
  private boolean isOutOfRange(@NonNull Calendar calendar) {
    UtilsGig.trimToMidnight(calendar);
    return isDisabled(calendar) || !isSelectable(calendar);
  }

  private boolean isDisabled(@NonNull Calendar c) {
    return disabledDays.contains(UtilsGig.trimToMidnight(c)) || isBeforeMin(c) || isAfterMax(c);
  }

  private boolean isSelectable(@NonNull Calendar c) {
    return selectableDays.isEmpty() || selectableDays.contains(UtilsGig.trimToMidnight(c));
  }
  ...
  @Override public @NonNull Calendar setToNearestDate(@NonNull Calendar calendar) {
    ...
    if (isBeforeMin(calendar)) {
      ...
      output.set(Calendar.MONTH, Calendar.JANUARY);
      output.set(Calendar.DAY_OF_MONTH, 1);
      return UtilsGig.trimToMidnight(output);
    }
