 ...
 */
public class RadialPickerLayoutGig extends FrameLayout implements OnTouchListener {
  private static final String TAG = "RadialPickerLayoutGig";
  ...
  private CircleViewGig mCircleViewGig;
  private AmPmCirclesViewGig mAmPmCirclesViewGig;
  private RadialTextsViewGIg mHourRadialTextsViewGIg;
  private RadialTextsViewGIg mMinuteRadialTextsViewGIg;
  private RadialTextsViewGIg mSecondRadialTextsViewGIg;
  private RadialSelectorViewGig mHourRadialSelectorViewGig;
  private RadialSelectorViewGig mMinuteRadialSelectorViewGig;
  private RadialSelectorViewGig mSecondRadialSelectorViewGig;
  ...
  public RadialPickerLayoutGig(Context context, AttributeSet attrs) {
    super(context, attrs);
    ...
    mCircleViewGig = new CircleViewGig(context);
    addView(mCircleViewGig);

    mAmPmCirclesViewGig = new AmPmCirclesViewGig(context);
    addView(mAmPmCirclesViewGig);

    mHourRadialSelectorViewGig = new RadialSelectorViewGig(context);
    addView(mHourRadialSelectorViewGig);
    mMinuteRadialSelectorViewGig = new RadialSelectorViewGig(context);
    addView(mMinuteRadialSelectorViewGig);
    mSecondRadialSelectorViewGig = new RadialSelectorViewGig(context);
    addView(mSecondRadialSelectorViewGig);

    mHourRadialTextsViewGIg = new RadialTextsViewGIg(context);
    addView(mHourRadialTextsViewGIg);
    mMinuteRadialTextsViewGIg = new RadialTextsViewGIg(context);
    addView(mMinuteRadialTextsViewGIg);
    mSecondRadialTextsViewGIg = new RadialTextsViewGIg(context);
    addView(mSecondRadialTextsViewGIg);
    ...
    mGrayBox.setBackgroundColor(ContextCompat.getColor(context,
        com.golrangsystem.materialdatetimepicker.R.color.fh_gig_mdtp_transparent_black));
  ...
  public void initialize(Context context, Locale locale,
    ...
    RadialTextsViewGIg.SelectionValidator secondValidator = selection -> {
      TimePointGig newTime =
          new TimePointGig(mCurrentTime.getHour(), mCurrentTime.getMinute(), selection);
      return !mController.isOutOfRange(newTime, SECOND_INDEX);
    };
    RadialTextsViewGIg.SelectionValidator minuteValidator = selection -> {
      TimePointGig newTime =
          new TimePointGig(mCurrentTime.getHour(), selection, mCurrentTime.getSecond());
      return !mController.isOutOfRange(newTime, MINUTE_INDEX);
    };
    RadialTextsViewGIg.SelectionValidator hourValidator = selection -> {
      TimePointGig newTime =
          new TimePointGig(selection, mCurrentTime.getMinute(), mCurrentTime.getSecond());
    ...
    mHourRadialTextsViewGIg.initialize(context, hoursTexts, (is24HourMode ? innerHoursTexts : null),
        mController, hourValidator, true);
    mHourRadialTextsViewGIg.setSelection(
        is24HourMode ? initialTime.getHour() : hours[initialTime.getHour() % 12]);
    mHourRadialTextsViewGIg.invalidate();
    mMinuteRadialTextsViewGIg.initialize(context, minutesTexts, null, mController, minuteValidator,
        false);
    mMinuteRadialTextsViewGIg.setSelection(initialTime.getMinute());
    mMinuteRadialTextsViewGIg.invalidate();
    mSecondRadialTextsViewGIg.initialize(context, secondsTexts, null, mController, secondValidator,
        false);
    mSecondRadialTextsViewGIg.setSelection(initialTime.getSecond());
    mSecondRadialTextsViewGIg.invalidate();
    ...
    mHourRadialSelectorViewGig.initialize(context, mController, is24HourMode, true, hourDegrees,
        isHourInnerCircle(initialTime.getHour()));
    int minuteDegrees = initialTime.getMinute() * MINUTE_VALUE_TO_DEGREES_STEP_SIZE;
    mMinuteRadialSelectorViewGig.initialize(context, mController, false, false, minuteDegrees,
        false);
    int secondDegrees = initialTime.getSecond() * SECOND_VALUE_TO_DEGREES_STEP_SIZE;
    mSecondRadialSelectorViewGig.initialize(context, mController, false, false, secondDegrees,
        false);
  ...
  /**
   * Set either the hour, the minute or the second. Will set the internal value, and set the
   * selection.
   */
  private void setItem(int index, TimePointGig time) {
  ...
  public TimePointGig getTime() {
    return mCurrentTime;
  }

  public void setTime(TimePointGig time) {
  ...
  public void setAmOrPm(int amOrPm) {
    mAmPmCirclesViewGig.setAmOrPm(amOrPm);
    mAmPmCirclesViewGig.invalidate();
    TimePointGig newSelection = new TimePointGig(mCurrentTime);
    if (amOrPm == AM) {
      newSelection.setAM();
   ...
   *
   * @param newSelection TimePointGig - Time which should be rounded
   * @param currentItemShowing int - The index of the current view
   * @return TimePointGig - the rounded value
   */
  private TimePointGig roundToValidTime(TimePointGig newSelection, int currentItemShowing) {
    switch (currentItemShowing) {
      case HOUR_INDEX:
        return mController.roundToNearest(newSelection, null);
      case MINUTE_INDEX:
        return mController.roundToNearest(newSelection, TimePointGig.TYPE.HOUR);
      default:
        return mController.roundToNearest(newSelection, TimePointGig.TYPE.MINUTE);
    }
  }
   ...
   * @param index The picker to use as a reference. Will be getCurrentItemShow() except when AM/PM
   * is changed
   * is on non-visible values, but use this to force the dot to be shown.
   */
  private void reselectSelector(TimePointGig newSelection, boolean forceDrawDot, int index) {
    switch (index) {
      case HOUR_INDEX:
        ...
        mHourRadialSelectorViewGig.setSelection(degrees, isInnerCircle, forceDrawDot);
        mHourRadialTextsViewGIg.setSelection(hour);
        ...
        if (newSelection.getMinute() != mCurrentTime.getMinute()) {
          ...
          mMinuteRadialSelectorViewGig.setSelection(minDegrees, isInnerCircle, forceDrawDot);
          mMinuteRadialTextsViewGIg.setSelection(newSelection.getMinute());
        }
        ...
        if (newSelection.getSecond() != mCurrentTime.getSecond()) {
          ...
          mSecondRadialSelectorViewGig.setSelection(secDegrees, isInnerCircle, forceDrawDot);
          mSecondRadialTextsViewGIg.setSelection(newSelection.getSecond());
        }
        break;
      case MINUTE_INDEX:
        ...
        mMinuteRadialSelectorViewGig.setSelection(degrees, false, forceDrawDot);
        mMinuteRadialTextsViewGIg.setSelection(newSelection.getMinute());
        // If we rounded the seconds, reposition the secondSelector too.
        if (newSelection.getSecond() != mCurrentTime.getSecond()) {
          int secDegrees = newSelection.getSecond() * (360 / 60);
          mSecondRadialSelectorViewGig.setSelection(secDegrees, false, forceDrawDot);
          mSecondRadialTextsViewGIg.setSelection(newSelection.getSecond());
        }
        break;
      case SECOND_INDEX:
        ...
        mSecondRadialSelectorViewGig.setSelection(degrees, false, forceDrawDot);
        mSecondRadialTextsViewGIg.setSelection(newSelection.getSecond());
    }
    ...
    switch (getCurrentItemShowing()) {
      case HOUR_INDEX:
        mHourRadialSelectorViewGig.invalidate();
        mHourRadialTextsViewGIg.invalidate();
        break;
      case MINUTE_INDEX:
        mMinuteRadialSelectorViewGig.invalidate();
        mMinuteRadialTextsViewGIg.invalidate();
        break;
      case SECOND_INDEX:
        mSecondRadialSelectorViewGig.invalidate();
        mSecondRadialTextsViewGIg.invalidate();
    }
  }

  private TimePointGig getTimeFromDegrees(int degrees, boolean isInnerCircle,
    ...
    boolean allowFineGrained =
        !forceToVisibleValue && (currentShowing == MINUTE_INDEX || currentShowing == SECOND_INDEX);
    if (allowFineGrained) {
    ...
    TimePointGig newSelection;
    switch (currentShowing) {
      case HOUR_INDEX:
        int hour = value;
        if (!mIs24HourMode && getIsCurrentlyAmOrPm() == PM && degrees != 360) hour += 12;
        if (!mIs24HourMode && getIsCurrentlyAmOrPm() == AM && degrees == 360) hour = 0;
        newSelection = new TimePointGig(hour, mCurrentTime.getMinute(), mCurrentTime.getSecond());
        break;
      case MINUTE_INDEX:
        newSelection = new TimePointGig(mCurrentTime.getHour(), value, mCurrentTime.getSecond());
        break;
      case SECOND_INDEX:
        newSelection = new TimePointGig(mCurrentTime.getHour(), mCurrentTime.getMinute(), value);
        break;
      default:
  ...
  private int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal,
    ...
    switch (getCurrentItemShowing()) {
      case HOUR_INDEX:
        return mHourRadialSelectorViewGig.getDegreesFromCoords(pointX, pointY, forceLegal,
            isInnerCircle);
      case MINUTE_INDEX:
        return mMinuteRadialSelectorViewGig.getDegreesFromCoords(pointX, pointY, forceLegal,
            isInnerCircle);
      case SECOND_INDEX:
        return mSecondRadialSelectorViewGig.getDegreesFromCoords(pointX, pointY, forceLegal,
            isInnerCircle);
      default:
        return -1;
    }
  ...
  public void setCurrentItemShowing(int index, boolean animate) {
    ...
    if (animate && (index != lastIndex)) {
      ...
      if (index == MINUTE_INDEX && lastIndex == HOUR_INDEX) {
        anims[0] = mHourRadialTextsViewGIg.getDisappearAnimator();
        anims[1] = mHourRadialSelectorViewGig.getDisappearAnimator();
        anims[2] = mMinuteRadialTextsViewGIg.getReappearAnimator();
        anims[3] = mMinuteRadialSelectorViewGig.getReappearAnimator();
      } else if (index == HOUR_INDEX && lastIndex == MINUTE_INDEX) {
        anims[0] = mHourRadialTextsViewGIg.getReappearAnimator();
        anims[1] = mHourRadialSelectorViewGig.getReappearAnimator();
        anims[2] = mMinuteRadialTextsViewGIg.getDisappearAnimator();
        anims[3] = mMinuteRadialSelectorViewGig.getDisappearAnimator();
      } else if (index == MINUTE_INDEX && lastIndex == SECOND_INDEX) {
        anims[0] = mSecondRadialTextsViewGIg.getDisappearAnimator();
        anims[1] = mSecondRadialSelectorViewGig.getDisappearAnimator();
        anims[2] = mMinuteRadialTextsViewGIg.getReappearAnimator();
        anims[3] = mMinuteRadialSelectorViewGig.getReappearAnimator();
      } else if (index == HOUR_INDEX && lastIndex == SECOND_INDEX) {
        anims[0] = mSecondRadialTextsViewGIg.getDisappearAnimator();
        anims[1] = mSecondRadialSelectorViewGig.getDisappearAnimator();
        anims[2] = mHourRadialTextsViewGIg.getReappearAnimator();
        anims[3] = mHourRadialSelectorViewGig.getReappearAnimator();
      } else if (index == SECOND_INDEX && lastIndex == MINUTE_INDEX) {
        anims[0] = mSecondRadialTextsViewGIg.getReappearAnimator();
        anims[1] = mSecondRadialSelectorViewGig.getReappearAnimator();
        anims[2] = mMinuteRadialTextsViewGIg.getDisappearAnimator();
        anims[3] = mMinuteRadialSelectorViewGig.getDisappearAnimator();
      } else if (index == SECOND_INDEX && lastIndex == HOUR_INDEX) {
        anims[0] = mSecondRadialTextsViewGIg.getReappearAnimator();
        anims[1] = mSecondRadialSelectorViewGig.getReappearAnimator();
        anims[2] = mHourRadialTextsViewGIg.getDisappearAnimator();
        anims[3] = mHourRadialSelectorViewGig.getDisappearAnimator();
      }
  ...
  private void transitionWithoutAnimation(int index) {
    ...
    mHourRadialTextsViewGIg.setAlpha(hourAlpha);
    mHourRadialSelectorViewGig.setAlpha(hourAlpha);
    mMinuteRadialTextsViewGIg.setAlpha(minuteAlpha);
    mMinuteRadialSelectorViewGig.setAlpha(minuteAlpha);
    mSecondRadialTextsViewGIg.setAlpha(secondAlpha);
    mSecondRadialSelectorViewGig.setAlpha(secondAlpha);
  }
    ...
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
          ...
          mHandler.postDelayed(() -> {
            mAmPmCirclesViewGig.setAmOrPmPressed(mIsTouchingAmOrPm);
            mAmPmCirclesViewGig.invalidate();
          }, TAP_TIMEOUT);
        } else {
          ...
          TimePointGig selectedTime = getTimeFromDegrees(mDownDegrees, isInnerCircle[0], false);
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
          ...
          int isTouchingAmOrPm = mAmPmCirclesViewGig.getIsTouchingAmOrPm(eventX, eventY);
          if (isTouchingAmOrPm != mIsTouchingAmOrPm) {
            mAmPmCirclesViewGig.setAmOrPmPressed(-1);
            mAmPmCirclesViewGig.invalidate();
            mIsTouchingAmOrPm = -1;
          }
          break;
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
          ...
          if (isTouchingAmOrPm == mIsTouchingAmOrPm) {
            mAmPmCirclesViewGig.setAmOrPm(isTouchingAmOrPm);
  ...
  @Override public boolean performAccessibilityAction(int action, Bundle arguments) {
    ...
    if (changeMultiplier != 0) {
      ...
      switch (currentItemShowing) {
        case HOUR_INDEX:
          newSelection =
              new TimePointGig(value, mCurrentTime.getMinute(), mCurrentTime.getSecond());
          break;
        case MINUTE_INDEX:
          newSelection = new TimePointGig(mCurrentTime.getHour(), value, mCurrentTime.getSecond());
          break;
  ...
  public interface OnValueSelectedListener {
    void onValueSelected(TimePointGig newTime);
