 ...
 */
public class RadialSelectorViewGig extends View {
  private static final String TAG = "RadialSelectorViewGig";
  ...
  private static final int FULL_ALPHA = UtilsGig.FULL_ALPHA;
  ...
  public RadialSelectorViewGig(Context context) {
    super(context);
  ...
  public void initialize(Context context, TimePickerControllerGIg controller,
    ...
    if (mIsInitialized) {
      Log.e(TAG, "This RadialSelectorViewGig may only be initialized once.");
      return;
    }
    ...
    if (hasInnerCircle) {
      ...
      mOuterNumbersRadiusMultiplier = Float.parseFloat(res.getString(
    ...
    mSelectionRadiusMultiplier = Float.parseFloat(res.getString(
        com.golrangsystem.materialdatetimepicker.R.string.fh_gig_mdtp_selection_radius_multiplier));
  ...
  public int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal,
    ...
    double hypotenuse = Math.sqrt(
        (pointY - mYCenter) * (pointY - mYCenter) + (pointX - mXCenter) * (pointX - mXCenter));
    ...
    if (mHasInnerCircle) {
      if (forceLegal) {
        ...
        int halfwayHypotenusePoint =
            (int) (mCircleRadius * ((mOuterNumbersRadiusMultiplier + mInnerNumbersRadiusMultiplier)
                / 2));
  ...
  public ObjectAnimator getDisappearAnimator() {
    if (!mIsInitialized || !mDrawValuesReady) {
      Log.e(TAG, "RadialSelectorViewGig was not ready for animation.");
      return null;
    }
  ...
  private class InvalidateUpdateListener implements AnimatorUpdateListener {
    @Override public void onAnimationUpdate(ValueAnimator animation) {
      RadialSelectorViewGig.this.invalidate();
    }
  }
}
