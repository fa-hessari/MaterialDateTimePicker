...
/**
 * An implementation of TimepointLimiter which implements the most common ways to restrict
 * Timepoints
 * in a TimePickerDialog
 * Created by wdullaer on 20/06/17.
 */
class DefaultTimepointLimiter implements TimepointLimiter {
  @SuppressWarnings("WeakerAccess") public static final Parcelable.Creator<DefaultTimepointLimiter>
      CREATOR = new Parcelable.Creator<DefaultTimepointLimiter>() {
  ...
  @SuppressWarnings("WeakerAccess") public DefaultTimepointLimiter(Parcel in) {
  ...
  @NonNull
  private TreeSet<Timepoint> getExclusiveSelectableTimes(@NonNull TreeSet<Timepoint> selectable,
      @NonNull TreeSet<Timepoint> disabled) {
    TreeSet<Timepoint> output = new TreeSet<>(selectable);
    output.removeAll(disabled);
    return output;
  }

  @Override public boolean isOutOfRange(@Nullable Timepoint current, int index,
    ...
    if (index == TimePickerDialog.HOUR_INDEX) {
      ...
      if (!exclusiveSelectableTimes.isEmpty()) {
        ...
        return !(current.equals(ceil, Timepoint.TYPE.HOUR) || current.equals(floor,
            Timepoint.TYPE.HOUR));
      }
      ...
      if (!exclusiveSelectableTimes.isEmpty()) {
        Timepoint ceil = exclusiveSelectableTimes.ceiling(current);
        Timepoint floor = exclusiveSelectableTimes.floor(current);
        return !(current.equals(ceil, Timepoint.TYPE.MINUTE) || current.equals(floor,
  ...
  @Override public @NonNull Timepoint roundToNearest(@NonNull Timepoint time,
      @Nullable Timepoint.TYPE type, @NonNull Timepoint.TYPE resolution) {
  ...
  private Timepoint searchValidTimePoint(@NonNull Timepoint time, @Nullable Timepoint.TYPE type,
      @NonNull Timepoint.TYPE resolution) {
